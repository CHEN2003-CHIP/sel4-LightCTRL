...
图 2-1 为 LightDemo 顶层工程架构图，展示 protection domain、shared memory 和主要消息路径。该图是工程架构图，不按 UML 组件图声明。
...
图 2-2 为控制链路 UML 顺序图，覆盖正常控制、故障上报、故障模式发布和状态查询路径。
...
sequenceDiagram
    ...
    S->>M: 更新 operator_request / target_output
    ...
    L->>F: 上报运行时 fault
    F->>M: 更新 fault lifecycle / fault mode
    F->>S: fault mode update
    F->>G: fault mode update
    S->>M: 重新计算安全 target output
    S->>L: 重新同步安全目标输出
    end

    rect rgb(245, 255, 245)
    U->>C: 输入状态查询 ?
    C->>M: 读取 light_shmem_t
    C-->>U: STATUS_SNAPSHOT 日志
    end
```
...
图 2-3 为 fault mode UML 状态机图。初始状态为 `NORMAL`，恢复路径按观察窗口逐级下降，不允许从高等级故障直接跳回 `NORMAL`。
...
图 2-4 为 fault lifecycle UML 状态机图。`faultmg` 是该生命周期的唯一 owner。
