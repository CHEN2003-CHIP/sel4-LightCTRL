...
static light_target_state_t requested_target_from_request(light_operator_request_t request,
    ...
    bool dark_environment = vehicle_state.ambient_light != LIGHT_VEHICLE_AMBIENT_DAY;
    bool hazard_active = vehicle_state.hazard != 0U
        || vehicle_state.drive_mode == LIGHT_VEHICLE_DRIVE_MODE_EMERGENCY;
    bool parking_context = vehicle_state.gear == LIGHT_VEHICLE_GEAR_PARK
        || vehicle_state.drive_mode == LIGHT_VEHICLE_DRIVE_MODE_PARKING;
    bool reverse_context = vehicle_state.gear == LIGHT_VEHICLE_GEAR_REVERSE;
    uint16_t high_beam_min_speed = vehicle_state.drive_mode == LIGHT_VEHICLE_DRIVE_MODE_HIGHWAY
        ? 30U
        : 10U;
    ...
    requested_target.position = marker_requested || dark_environment || parking_context;
    ...
    } else if (!brake_active && vehicle_state.speed_kph <= 120U) {
    ...
    if (low_requested || dark_environment || reverse_context) {
        requested_target.low_beam = true;
    }

    if (high_requested && low_requested && !brake_active && !parking_context
        && !reverse_context && vehicle_state.speed_kph >= high_beam_min_speed) {
        requested_target.low_beam = false;
