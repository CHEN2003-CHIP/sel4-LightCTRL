...
static const char *lamp_word(uint8_t enabled) {
    return enabled != 0U ? "ON" : "off";
}

static const char *mode_color(fault_mode_t mode) {
    switch (mode) {
...
static void log_command_result(light_control_command_result_t result) {
    if (!result.accepted) {
        LOG_INFO("SCHED_CMD_REJECT cmd=0x%02x reason=%d",
                 ...
                 (int)result.reason);
        LOG_INFO("DEMO_RESULT stage=scheduler status=REJECT cmd=0x%02x reason=%d",
                 (unsigned int)result.cmd,
                 (int)result.reason);
        return;
    }
    ...
    LOG_INFO("DEMO_RESULT stage=scheduler status=ACCEPT cmd=0x%02x request=low:%u,high:%u,left:%u,right:%u,marker:%u,brake:%u",
             (unsigned int)result.cmd,
             (unsigned int)result.next_request.low_beam_req,
             (unsigned int)result.next_request.high_beam_req,
             (unsigned int)result.next_request.left_turn_req,
...
static void recompute_target_output(void) {
    ...
    LOG_INFO("SCHED_TARGET mode=%s speed=%u ignition=%u brake_pedal=%u gear=%u ambient=%u hazard=%u drive_mode=%u req[low=%u high=%u left=%u right=%u marker=%u brake=%u] target[low=%u high=%u left=%u right=%u marker=%u brake=%u]",
             ...
             (unsigned int)g_shmem->vehicle_state.gear,
             (unsigned int)g_shmem->vehicle_state.ambient_light,
             (unsigned int)g_shmem->vehicle_state.hazard,
             (unsigned int)g_shmem->vehicle_state.drive_mode,
    ...
    LOG_INFO("DEMO_RESULT stage=scheduler mode=%s vehicle=speed:%u,gear:%u,ambient:%u,hazard:%u output=low:%u,high:%u,left:%u,right:%u,marker:%u,brake:%u",
             light_fault_mode_name((fault_mode_t)g_shmem->fault_mode),
             (unsigned int)g_shmem->vehicle_state.speed_kph,
             (unsigned int)g_shmem->vehicle_state.gear,
             (unsigned int)g_shmem->vehicle_state.ambient_light,
             ...
             (unsigned int)target_output.marker_on,
             (unsigned int)target_output.brake_on);
    LOG_INFO("%s>>> RESULT%s mode=%s%s%s speed=%u lamps LOW=%s HIGH=%s LEFT=%s RIGHT=%s MARKER=%s BRAKE=%s",
             DEMO_ANSI_CYAN,
             DEMO_ANSI_RESET,
             mode_color((fault_mode_t)g_shmem->fault_mode),
             light_fault_mode_name((fault_mode_t)g_shmem->fault_mode),
             DEMO_ANSI_RESET,
             (unsigned int)g_shmem->vehicle_state.speed_kph,
             lamp_word(target_output.low_beam_on),
             lamp_word(target_output.high_beam_on),
             lamp_word(target_output.left_turn_on),
             lamp_word(target_output.right_turn_on),
             lamp_word(target_output.marker_on),
             lamp_word(target_output.brake_on));
}

void init(void) {
    g_shmem = (light_shmem_t *)shared_memory_base_vaddr;

    g_shmem->layout_version = LIGHT_SHARED_STATE_LAYOUT_CURRENT;
    ...
    g_shmem->fault_recovery_elapsed_ms = 0U;
    ...
    if (g_shmem->vehicle_state.ignition_on == 0U && g_shmem->vehicle_state.speed_kph == 0U
        && g_shmem->vehicle_state.brake_pedal == 0U && g_shmem->vehicle_state.gear == 0U
        && g_shmem->vehicle_state.ambient_light == 0U && g_shmem->vehicle_state.drive_mode == 0U) {
...
void notified(microkit_channel ch) {
    ...
    if (ch == LIGHT_CH_SCHEDULER_FROM_COMMANDIN) {
        ...
        if (contract.status != LIGHT_CONTRACT_OK) {
            LOG_INFO("SCHED_CONTRACT_REJECT reason=%s expected=%u actual=%u type=%u len=%u version=%u",
                     light_contract_status_name(contract.status),
                     (unsigned int)contract.expected,
                     (unsigned int)contract.actual,
