...
static void test_policy_matrix_matches_safe_mode_semantics(void) {
    ...
    expect_true(matrix->turn_left == LIGHT_OUTPUT_RULE_FORCE_OFF, "SAFE_MODE turn-left should be forced off");
    expect_true(matrix->turn_right == LIGHT_OUTPUT_RULE_FORCE_OFF, "SAFE_MODE turn-right should be forced off");
...
static void test_safe_mode_forces_conservative_output_profile(void) {
    ...
    expect_true(!result.target.turn_left, "SAFE_MODE should disable turn-left output");
...
static void test_safe_mode_clamps_repeated_high_beam_and_turn_requests(void) {
    light_target_state_t request = requested_target(false, true, false, false, true, false);
    light_output_policy_result_t first =
        light_output_policy_apply(request, LIGHT_FAULT_MODE_SAFE_MODE);
    light_output_policy_result_t second =
        light_output_policy_apply(request, LIGHT_FAULT_MODE_SAFE_MODE);

    expect_true(first.changed, "SAFE_MODE should clamp repeated risky requests on first pass");
    expect_true(second.changed, "SAFE_MODE should clamp repeated risky requests on every pass");
    expect_true(!first.target.turn_left && !second.target.turn_left,
                "SAFE_MODE should keep turn-left off across repeated requests");
    expect_true(!first.target.high_beam && !second.target.high_beam,
                "SAFE_MODE should keep high beam off across repeated requests");
    expect_true(first.target.low_beam && second.target.low_beam,
                "SAFE_MODE should keep low beam on across repeated requests");
    expect_true(first.target.position && second.target.position,
                "SAFE_MODE should keep position light on across repeated requests");
}
...
static void test_safe_mode_blocks_high_beam_actions(void) {
    light_runtime_guard_context_t context = default_guard_context();
    light_runtime_guard_result_t result;
    ...
    expect_true(!result.allowed, "SAFE_MODE should block high beam actions");
    expect_true(result.report_fault, "SAFE_MODE high beam block should report fault");
    expect_true(result.error_code == LIGHT_ERR_HW_STATE_ERR,
                "SAFE_MODE high beam block should use hardware-state error");
}
...
int main(void) {
    ...
    test_safe_mode_clamps_repeated_high_beam_and_turn_requests();
    test_degraded_mode_blocks_high_beam();
    test_safe_mode_blocks_turn_actions();
    test_safe_mode_blocks_high_beam_actions();
